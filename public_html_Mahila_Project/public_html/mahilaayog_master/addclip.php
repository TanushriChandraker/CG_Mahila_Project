...
if (isset($_SESSION['id']) && isset($_SESSION['username'])) {
	if(isset($_POST['submit'])){
		...
		// image upload code
			...
			foreach ($arrImages as $key => $arrImage) {
				...
				$Filename = 'clip' . "_" . rand(1000000000, 9999999999) . "." . $file_ext;
	...
	$clipdate = mysqli_real_escape_string($conn,$_POST['clipdate']);
	...
	if (empty($clipdate)) {
		header("Location: addclip.php?error=field shuld not be blank");
	    exit();
	}else{
			$query = mysqli_query($conn, "insert into newsclips(clipdate,files,Is_Active,created_at) values('$clipdate','$file','$status','$created_at')");
			// print_r($query);
			// die();
			if($query){
				$file = explode(",", $file);
						foreach($file as $key => $singleFile){
							move_uploaded_file($tmp_names[$key], 'upload/newsclips/'.$singleFile);
							}
				header("Location: addclip.php?success=Data Added successfully");
			}else{
				header("Location: addclip.php?error=OOPS Something Went Wrong");
			}
		}
	}
					...
					<div class="card-body">
						<div class="row">
							<form method="post" enctype='multipart/form-data'>
								...
								<div class="formbar">
									<div class="col-sm-3">
										<input type="date" class="form-control" name="clipdate" required>
									</div><!-- 
									<div class="col-sm-3">
									...
									<div class="col-sm-3">
										<input type="file" name="imagefile[]" class="form-control" multiple="multiple" required>
									</div>
								</div>
							</div>
