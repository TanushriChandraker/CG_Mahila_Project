...
if (isset($_SESSION['id']) && isset($_SESSION['username']) && isset($_SESSION['role'])) {
	...
	if($act=='del'){
	$id = intval($_GET['del']);
	$res = "SELECT * FROM passed_event where id=$id";
        $sql = mysqli_query($conn, $res) or die("Query failed : select");
        $row = mysqli_fetch_assoc($sql);
        // print_r($row['files']);
          $row['files'] = explode(',', $row['files']);
        foreach($row['files'] as $file){
        unlink("upload/passed-event/".$file);  	
        }
        // die();
	$query = mysqli_query($conn, "delete from passed_event where id = '$id'");
	if($query){
		header("Location: passlist.php?success = Record Deleted Successfuly");
	}else{
		header("Location: passlist.php?error = OOPS Something Went Wrong");
	}
}
 ?>
			...
			<div class="container">
				...
				<div style="overflow-x: auto;">
					<table >
						...
						<tbody>
							<?php
							$query="select*from passed_event";
							...
							<tr>
								...
								<td><?php echo htmlentities($row['passdate']);?></td>
								<td><?php echo htmlentities($row['passhed']);?></td>
								...
								<?php }} ?>
								<?php }?></td>
								<td><a href="passedit.php?edit=<?php echo ($row['id']);?>"><input type="button" class="btn btn-primary" value="Edit"></a>
								<a href="passlist.php?del=<?php echo ($row['id']);?>&action=del"><input type="button" class="btn btn-danger" value="Delete"></td>
							</tr>
							<?php
							$cnt++;
 							} ?>
