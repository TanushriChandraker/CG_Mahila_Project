...
if (isset($_SESSION['id']) && isset($_SESSION['username'])) {
	if(isset($_POST['submit'])){
		...
		// image upload code
			...
			foreach ($arrImages as $key => $arrImage) {
				...
				$Filename = 'upcoming' . "_" . rand(1000000000, 9999999999) . "." . $file_ext;
	...
	if (empty($update)) {
		header("Location: addup.php?error=field shuld not be blank");
	    exit();
	}else if(empty($uphed)){
		header("Location: addup.php?error=field shuld not be blank");
	    	exit();
	}else{
			$query = mysqli_query($conn, "insert into upcoming_event(up_date,uphed,public,files,Is_Active,created_at) values('$update','$uphed','$public','$file','$status','$created_at')");
			// print_r($query);
			// die();
			if($query){
				$file = explode(",", $file);
						foreach($file as $key => $singleFile){
							move_uploaded_file($tmp_names[$key], 'upload/upcoming-event/'.$singleFile);
							}
				header("Location: addup.php?success=Data Added successfully");
			}else{
				header("Location: addup.php?error=OOPS Something Went Wrong");
			}
		}
	}
						...
						<div class="row">
							<form method="post" enctype='multipart/form-data'>
								...
								<div class="formbar">
									...
									<div class="col-sm-3">
										<input type="text" class="form-control" name= "uphed" placeholder="हेडिंग" aria-label="" required>
									</div><!-- 
									<div class="col-sm-5">
